...
This repository keeps the command reference and helper scripts, not generated
crawl outputs. Recreate local evidence in a temporary directory when refreshing
the snapshot.
...
atlcli --version > "$EVIDENCE_DIR/atlcli-version-$TAG.txt"
atlcli --help > "$EVIDENCE_DIR/atlcli-help-$TAG.txt"
curl -fsSL https://atlcli.sh/sitemap-index.xml > "$EVIDENCE_DIR/sitemap-index-$TAG.xml"
curl -fsSL https://atlcli.sh/sitemap-0.xml > "$EVIDENCE_DIR/sitemap-0-$TAG.xml"
"$SKILL_DIR/scripts/extract_doc_commands.py" "$EVIDENCE_DIR" --tag "$TAG" > "$EVIDENCE_DIR/extract-run-$TAG.txt"
```
...
- Re-run reproduction commands and compare local evidence before editing this file.
